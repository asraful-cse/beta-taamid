import React, { useEffect, useState } from 'react';
import useWebSocket, { ReadyState } from 'react-use-websocket';
import { useDispatch, useSelector } from "react-redux";
import { Link, useParams } from "react-router-dom";
import { GetGroupChatData } from "../../_redux/Action";
import './ChatBot.css'
import send from '../../../../images/myPo/navigate.png';
import LastChats from "../LastChats/LastChats";
import { useTranslation } from "react-i18next";



const ChatBot = () => {
    const { t } = useTranslation();

    const { companyId } = useParams()

    const token = localStorage.getItem('access_token')


    //Public API that will echo messages sent to it back to the client

    const webSocketUrl = window.location.protocol == "https:" ? `wss://dev.taamid.com/ws/chat?token=${token}` : `ws://134.209.193.73/ws/chat?token=${token}`
    const [socketUrl, setSocketUrl] = useState(webSocketUrl);
    const [messageHistory, setMessageHistory] = useState([]);


    const {
        sendJsonMessage,
        lastMessage,
        readyState,
    } = useWebSocket(socketUrl);

    useEffect(() => {
        if (lastMessage !== null) {
            const jsonData = JSON.parse(lastMessage.data)
            setMessageHistory(prev => prev.concat(jsonData.receive_dict));
        }

    }, [lastMessage]);

    const [text, setText] = useState('')

    const handleClickSendMessage = () => {

        sendJsonMessage({ message: text, receiver_company: companyId })
        // document.getElementById('msgAreaId').value='';
        setText('');

    }

    // const connectionStatus = {
    //     [ReadyState.CONNECTING]: 'Connecting',
    //     [ReadyState.OPEN]: 'Open',
    //     [ReadyState.CLOSING]: 'Closing',
    //     [ReadyState.CLOSED]: 'Closed',
    //     [ReadyState.UNINSTANTIATED]: 'Uninstantiated',
    // }[readyState];


    const groupChatInfo = useSelector((state) => state.myPoInfo.groupChatData);

    if (groupChatInfo) {
        localStorage.setItem('recentChatCompany', groupChatInfo?.data[0]?.other_company)

    }

    // company Name filtering
    const [companyName, setCompanyName] = useState('')
    var eachCompanyChat = ''

    useEffect(() => {
        if (groupChatInfo) {
            eachCompanyChat = groupChatInfo.data.filter(item => parseInt(item.other_company) === parseInt(companyId)).map(item =>
                item.company_name)
            setCompanyName(eachCompanyChat[0])
        }

    }, [groupChatInfo, companyId])


    // dispatching group chat
    let other_company = companyId;
    var eachGroupChat = []
    const dispatch = useDispatch();
    useEffect(() => {
        dispatch(GetGroupChatData());

    }, []);


    // eachGroupChat filtering group groupInfoChat

    useEffect(() => {

        if (groupChatInfo) {
            eachGroupChat = groupChatInfo.data.filter(function (c) {
                if (c.other_company == other_company) {
                    return c;
                }
            }).map(function (x) {
                return x.group_messages;
            });
            eachGroupChat = eachGroupChat[0];
            setMessageHistory(() => eachGroupChat)
        }

    }, [companyId, groupChatInfo]);


    return (
        <>
            <div className="row">

                <div className="col-md-8 col-lg-8 col-xl-8 my-5">
                    <div className="card">
                        <div className="card-header msg_head msgHeader">
                            <div className="bd-highlight">

                                <h6 className="user_info mt-3 text-capitalize">
                                    {companyName}
                                </h6>

                                <div className="video_cam">
                                    <span><i className="fas fa-video"> </i> </span>
                                    <span><i className="fas fa-phone"> </i> </span>
                                </div>
                            </div>
                        </div>
                        <div style={{ backgroundColor: '#FCFCFD' }} className="card-body msg_card_body">
                            {messageHistory &&
                                messageHistory.map(otherCompanyMsg =>
                                (

                                    otherCompanyMsg.sender_company === companyId ?

                                        (<div className="d-flex justify-content-start mb-4">
                                            <div className="img_cont_msg">
                                                <img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg"
                                                    alt='p' className="rounded-circle user_img_msg" />
                                            </div>
                                            <div className="msg_cotainer">
                                                {otherCompanyMsg.message_text}

                                                <span className="msg_time">8:40 AM, Today</span>
                                            </div>
                                        </div>) :
                                        (
                                            <div className="d-flex justify-content-end mb-4">

                                                <div className="msg_cotainer_send">
                                                    {otherCompanyMsg.message_text}

                                                    <span className="msg_time_send">8:55 AM, Today</span>
                                                </div>

                                                <div className="img_cont_msg">
                                                    <img
                                                        src="data:image/jpeg;base64,"
                                                        className="rounded-circle user_img_msg" alt='p2' />

                                                </div>
                                            </div>
                                        )


                                )
                                )
                            }


                        </div>
                        <div style={{}} className="card-footer msgFooter">
                            <div className="input-group">
                                <div className="input-group-append">
                                    <span className="input-group-text attach_btn"><i
                                        className="fas fa-paperclip"> </i> </span>
                                </div>
                                <textarea style={{ paddingTop: '15px' }} id='msgAreaId' name=""
                                    className="form-control type_msg"
                                    placeholder={t("Type your message...")} value={text}
                                    onChange={(e) => setText(e.target.value)}> </textarea>
                                <div className="input-group-append">
                                    <span className="input-group-text send_btn primary-btn"
                                        onClick={handleClickSendMessage}
                                        disabled={readyState !== ReadyState.OPEN}>
                                        <img className='sendBg' src={send} alt="send" />
                                    </span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>


                <div className="col-md-4 col-lg-4 col-xl-4 my-5">
                    <div className="lastChatWrapper">
                        <h2 className='lastChatTitle'>{t("Last Chats")}</h2>

                        <ul className='customChatUl'>
                            <li style={{ listStyle: 'none', cursor: 'pointer' }}>
                                {
                                    groupChatInfo && groupChatInfo.data.map(
                                        lastChat =>
                                            <Link to={`/messageCenter/${lastChat.other_company}`}> <LastChats
                                                lastChat={lastChat} /></Link>
                                    )
                                }

                            </li>
                        </ul>

                    </div>

                </div>
            </div>
        </>
    );
};

export default ChatBot;