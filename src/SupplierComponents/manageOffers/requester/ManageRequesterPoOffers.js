...
const ManageRequesterPoOffers = () => {
  const [status, statusSet] = useState('pending');
      ...
      <div className='row'>
        {items && items.map((item, index) => (
          <Link
            ...
            to={
              status === "pending" ?
                `/requester-manage-offer/pending/${item?.id}` :
                status === "completed" ?
                  `/requester-manage-offer/completed/${item?.id}` :
                  `/requester-manage-offer/accepted/${item?.id}`
            }
...
export default ManageRequesterPoOffers;